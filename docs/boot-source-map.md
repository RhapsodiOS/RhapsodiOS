...
| Drivers | `src/kernel-7/bsd/kern/init_main.c` calls `autoconf()` when `DRIVERKIT` is enabled. `src/kernel-7/driverkit/i386/autoconf_i386.m` supplies i386 `probeNativeDevices()`, `probeHardware()`, and `probeDirectDevices()`. | The same call site additionally calls `bsd_autoconf()` when `ppc` is defined; `src/kernel-7/machdep/ppc/swapgeneric.m` defines it. | `src/kernel-7/driverkit/autoconfCommon.m` `autoconf()` initializes the I/O task and library functions, starts `autoconfInt` in that task, and waits for completion; `autoconfInt()` orders native, hardware, direct, and pseudo-device probes. i386 `probeNativeDevices()` reads `KERNBOOTSTRUCT.config`; its hardware/direct probe functions are empty. | Verified for conditional calls, i386 probe source, and probe ordering; **Research gap** for exact project build/install selection under `src/drivers/` and for a concrete boot configuration. | Trace build/install manifests and a target boot configuration for a concrete driver inventory. |
...
the PowerMac project builds that product from `powermac_init.c`. The
remaining research gap is whether a particular loaded kernel image was built
from this configuration and artifact.
