...
Before it registers the boot-loaded drivers, `probeNativeDevices()` calls the platform expert's `pexpert_init()`. That reads the ACPI static tables (RSDP, RSDT or XSDT, MADT, FADT, HPET, MCFG) and the MP table, publishes them in `i386_firmware_info`, hands the PCI configuration accessors the MCFG space, and, when the boot line carries `apic=1`, replaces the 8259 pair with the local and I/O APICs (then `lapictimer=1` moves the tick to the local APIC timer through `clock_set_tick_source()`, `smp=1` starts the other processors and parks them, and `acpi=1` enters ACPI mode with the power button on the SCI): `intr.c` keeps its dispatch table, ipl masks and deferral and drives whichever `intr_controller_t` is installed, on 64 irqs over the IDT vectors 0x40–0x7F it already had. `findBootConfigString()` also serves the tables linked into the kernel after the booter's, which is how the platform expert's own "Bus Class" table reaches the bus probe. **Source anchor:** `src/drivers-i386/bus/drvPExpert/i386/pexpert.c` `pexpert_init()`, `apic_intr.c` `apic_intr_enable()`; `src/kernel-7/machdep/i386/intr.c` `intr_set_controller()`; `src/kernel-7/driverkit/i386/autoconf_i386.m` `inKernelConfigTables`.
