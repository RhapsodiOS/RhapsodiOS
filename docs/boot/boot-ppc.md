...
The kernel entry comment says that Open Firmware loaded `SecondaryLoader`, which loaded a Mach-O `MH_PRELOAD` image and jumped to `start`; it also records expected supervisor mode, translations off, and `ARG0` as startup parameters. **Verified loader binding:** `SecondaryLoader` uses its BootInfo `kernel` property when supplied, otherwise Open Firmware's `boot-file`, then passes the selected file descriptor to `loadprog()`. `loadprog()` returns `kernelAddr` and `gKernelEntryP`, and the loader disables translations before transferring through that returned entry. **Research gap:** no tracked kernel image, build log, link map, or manifest proves which exact file was selected at a particular boot or ties it to a particular build artifact. **Source anchor:** `src/boot-2/ppc/SecondaryLoader/SecondaryInC.c` BootInfo `kernel`, `boot-file`, `loadprog()`, `gKernelEntryP`, `kernelAddr`; `src/kernel-7/machdep/ppc/start.s` `start` comment.
...
The material PPC gap is not a missing `ppc_init` definition, missing checked-in build selection, or an unknown loader-to-entry mechanism: the PowerMac PExpert source supplies it, its project builds `pexpertpowermac.o` from `powermac_init.c`, the PPC kernel configuration links that object, and SecondaryLoader binds its selected kernel file to the `loadprog()`-returned entry before transferring control. What remains unverified is provenance of a particular loaded kernel image, installed driver selection, concrete root device/filesystem, and installed bootstrap configuration. **Source anchor:** `src/kernel-7/conf/MASTER.ppc` `LIBPEXPERT`; `src/kernel-7/conf/Makefile.ppc` `LDOBJS_SUFFIX`; `src/drivers/ppc/bus/drvPExpert/powermac/powermac_init.c` `ppc_init`; `src/boot-2/ppc/SecondaryLoader/SecondaryInC.c` `loadprog()`, `gKernelEntryP`, `kernelAddr`; `docs/boot-source-map.md` “Drivers”, “Root filesystem”, “First user process”.
