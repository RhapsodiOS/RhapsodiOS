...
- Define every `__const` and `__data` symbol named in the Global Constraints Cirrus list, with those exact names. The 24 `_GD5434_mode_*` and `_GD5446_mode_*` records are `const`; `_GD5434_modeTable` and `_GD5446_modeTable` are mutable and live in `__DATA,__data`, which means they are non-const file-scope arrays populated at runtime by `determineConfiguration`.
