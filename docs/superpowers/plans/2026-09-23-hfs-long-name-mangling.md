...
- Never modify `/build/src`, `/build/tools`, `/build/repo` or `/build/state` on the box. Never start anything while another `rbuild` runs there. Private directories are `/build/hfs-name-*` and are removed when done.
