/*
 * Intel82556NetworkDriver.m
 * Intel EtherExpress PRO/100 EISA Network Driver
 */

#import "Intel82556NetworkDriver.h"
...
@implementation Intel82556NetworkDriver
...
{
    Intel82556NetworkDriver *driver;
    BOOL result = NO;
...
- initFromDeviceDescription:(IODeviceDescription *)deviceDescription
{
    ...
    if ([deviceDescription isKindOf:[IOEISADeviceDescription class]]) {
        ...
        IOLog("Intel82556: Found device at I/O base 0x%x, IRQ %d\n",
              _ioBase, _irqLevel);
    } else {
        IOLog("Intel82556: Invalid device description\n");
        [self free];
        return nil;
    }
    ...
    if (![self getHardwareAddress:(enet_addr_t *)_romAddress]) {
        IOLog("Intel82556: Failed to read hardware address\n");
        [self free];
        return nil;
    }

    IOLog("Intel82556: MAC address %02x:%02x:%02x:%02x:%02x:%02x\n",
          _romAddress[0], _romAddress[1], _romAddress[2],
          _romAddress[3], _romAddress[4], _romAddress[5]);

    /* Allocate buffers */
    if (![self allocateBuffers]) {
        IOLog("Intel82556: Failed to allocate buffers\n");
        [self free];
        return nil;
    }
    ...
    /* Initialize the chip */
    if (![self initChip]) {
        IOLog("Intel82556: Failed to initialize chip\n");
        [self free];
        return nil;
    }
...
- (BOOL)resetAndEnable:(BOOL)enable
{
    if (enable) {
        ...
        if (![self initChip]) {
            IOLog("Intel82556: Failed to initialize chip during enable\n");
            return NO;
        }
...
- (void)timeoutOccurred
{
    ...
    if (_transmitTimeout > COMMAND_TIMEOUT) {
        IOLog("Intel82556: Transmit timeout, resetting\n");
...
- (int)performCommand:(unsigned int)cmd
{
    ...
    if (timeout <= 0) {
        IOLog("Intel82556: Command timeout\n");
        return -1;
    }
...
- (BOOL)checkLink
{
    ...
    if (status >= 0) {
        ...
        if (_linkUp) {
            IOLog("Intel82556: Link is up\n");
        } else {
            IOLog("Intel82556: Link is down\n");
        }
