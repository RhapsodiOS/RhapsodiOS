 ...
 *        - foo-hdrs              -> foo's "headers" node (make installhdrs).
 *        - foo / foo-obj         -> foo's "full" node    (make install).
 *        - unknown dependencies  -> warned about and skipped.
 *   5. Emits build.ninja with two nodes per project (headers + full),
 *      a build-base aggregate, and buildworld / buildkernel phonies.
 *
 * Portability: POSIX C (dirent.h, sys/stat.h). Build with any cc on a POSIX
 * host (Rhapsody/Darwin, macOS, Linux). See ninja/README.md.
 */
...
struct config {
	...
	const char *outfile;   /* build.ninja output path           */
};
...
static void emit(const struct config *cfg)
{
	...
	fprintf(f, "# build.ninja - generated by genninja. DO NOT EDIT.\n");
	fprintf(f, "# Regenerate with: ninja/genninja (see ninja/README.md)\n\n");
...
int main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		const char *a = argv[i];
		#define NEXTARG() (++i < argc ? argv[i] : (usage(), exit(1), (char*)0))
		if      (!strcmp(a, "--srcroot"))  cfg.srcroot  = NEXTARG();
		else if (!strcmp(a, "--dstroot"))  cfg.dstroot  = NEXTARG();
		else if (!strcmp(a, "--objroot"))  cfg.objroot  = NEXTARG();
		...
		else if (!strcmp(a, "-o") || !strcmp(a, "--out")) cfg.outfile = NEXTARG();
		else if (!strcmp(a, "-h") || !strcmp(a, "--help")) { usage(); return 0; }
		else { fprintf(stderr, "genninja: unknown option '%s'\n", a); usage(); return 1; }
		#undef NEXTARG
	}

	if (!cfg.toolroot)
		cfg.toolroot = cfg.dstroot;
