 ...
 * All rights reserved.
 *
 * ProgramDAC.m -- DAC support for the Cirrus Logic GD5434 display driver.
 *
 * The GD5434 palette DAC is six bits per component. The write index is set
 * once, at port 0x3C8, and auto-increments after every third write to the
 * data register at 0x3C9, so the three writes that make up one entry must
 * stay adjacent and in red, green, blue order.
 */
#import "CirrusLogicGD5434DisplayDriver.h"
#import <driverkit/generalFuncs.h>
#import <driverkit/i386/ioPorts.h>

/* The `ProgramDAC' category of `CirrusLogicGD5434DisplayDriver'. */

@implementation CirrusLogicGD5434DisplayDriver (ProgramDAC)
...
/* Default gamma precompensation tables for color displays.
 ...
 * ramp values are scaled by the brightness and reduced to the six bits the
 * DAC keeps, so a level of EV_SCREEN_MAX_BRIGHTNESS gives a full ramp.
 */
static void
...
- setGammaTable
{
	...
	case IO_15BitsPerPixel:
	    /* Only the top four bits of each component index the palette,
	     * so each of the sixteen ramp values is written sixteen times.
	     */
	    for (i = 0; i < 16; i++) {
		for (j = 0; j < 16; j++) {
		    SetGammaValue(gamma16[i], gamma16[i], gamma16[i],
				  brightnessLevel);
		}
	    }
	    break;
