...
none of the reference's behaviour.** Our `CirrusLogicGD5434DisplayDriver.lksproj`
implements a class subclassing `IOPCIDirectDevice`; Apple's implements
...
classes. They share no string and no ivar.
...
reports five of the 21 reference functions as `mapped`:
...
The collision is **nominal only**. A shared selector name is not evidence of a
shared implementation, and the mapped count is therefore not the measure of
disjointness — the absence of any shared behaviour is, and that holds for all 21
functions including these five. Every one still has to be written from scratch.
This document records **the reference's behaviour**, not a function-by-function
diff. There is nothing to diff until Tasks 3 and 4 write the replacements.
...
The five `mapped` are the nominal name collisions at 0, 712, 804, 3296 and 3544
...
**Nothing was moved by hand**, in either direction. The five collisions were left
in `mapped` rather than being suppressed by renaming our invented source, because
the buckets should report what the tool observed.

Once `ProgramDAC.m` lands (Task 3), the committed `source-map.json` alongside this
document reads **`mapped 9` / `unmapped 12`** — the same five plus the four real
reconstructions at 3588, 3924, 4004 and 4088. Those four *are* implementations of
the reference; the five are not.
