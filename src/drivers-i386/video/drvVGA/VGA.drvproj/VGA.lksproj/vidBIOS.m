...
/* Finding 30.  Every failure path returns [self free], which is nil; the
 * compiler cross-jumps the four of them into one tail, which is why there
 * is no shared `fail' label here -- writing one costs a stack adjustment
 ...
 * here.
 *
 * inregs and outregs are routinely the same block -- that is what
 * -[IOVGADisplay(VESAMode) int10:] hands us -- so the incoming registers
 * are copied into a local before emu486 is allowed to write anything back.
 */
