...
**Reading the line numbers.** Each finding below was written during Phase 1,
against the source as it stood then. Phase 2's repairs moved most of
`SCSITape.m`, so the `SCSITape.m:NNN` and `SCSITapeKern.m:NNN` citations
*inside* finding bodies point at the pre-repair source and are left that way
deliberately — they are part of the record of what was found. The two finding
*headings* that carry a line range have been re-anchored to the current file,
and the authoritative current anchor for any function is its `source_line` in
`SCSITape/source-map.json` and `SCSITape/ledger.json`, both regenerated by
Task 12 from the finished tree.

## Starting state

Before any Phase 2 repair, as Task 1 generated it:
...
Task 12's regeneration from the finished tree gives `SCSITape_reloc` 48
mapped / 2 unmapped and `stblocksize` 4 / 16; `PreLoad` and `PostLoad` are
unchanged. See "Acceptance" at the end of this document.

## Summary

**Phase 1** tally across all four ledgers, as it stood at the end of Task 7 —
...
**Current** tally, after Phase 2 (Tasks 8-11) repaired every finding and Task
12 regenerated the artifacts from the finished tree. No entry is `unexamined`:

| Ledger | `assembly-matched` | `intentional-mismatch` | `unexamined` | Total |
| --- | --- | --- | --- | --- |
| `SCSITape_reloc` | 48 | 2 | 0 | 50 |
| `PreLoad` | 1 | 11 | 0 | 12 |
| `PostLoad` | 1 | 15 | 0 | 16 |
| `stblocksize` | 4 | 16 | 0 | 20 |
| **Total** | **54** | **44** | **0** | **98** |

Each finding below carries a disposition in its heading, and the "Phase 2
outcome" section near the end of this document maps every one to the commit
that repaired it. The findings themselves are left as Phase 1 wrote them:
this is the record of what was found, not a description of the current source.
...
*Disposition:* Task 9 wrote both (`c44c5c53`), in a new `SCSITape(private)`
category, and Task 12's regenerated `SCSITape/source-map.json` maps both to
their definitions.
...
*Disposition:* all 14 are now `assembly-matched`. Tasks 8 and 11 repaired the
ten source divergences, Task 9 wrote the two absent methods, and Tasks 10 and
12 mapped the two the scanner had wrongly excluded. The 2 build-generated
classes remain `intentional-mismatch` permanently. See "Phase 2 outcome"
below for the finding-to-commit mapping.
...
| 36 | `+[SCSITape probe:]` | `assembly-matched` (Phase 2) | Finding: queries `[controllerId numberOfTargets]` and clamps/warns instead of using the compile-time `SCSI_NTARGETS`; Finding: no target/lun reservation call anywhere in the function |
| 1544 | `-[SCSITape free]` | `assembly-matched` (Phase 2) | Finding: calls `[self releaseDevice]` guarded on `_devAcquired` and never touches `_reservedTargetLun`/`_controller`/`releaseTarget:lun:forOwner:` |
...
| 2248 | `-[SCSITape setIgnoreCheckCondition:]` | `assembly-matched` (Phase 2) | Finding: writes to `0x225(r3)` — resolved via `__OBJC,__instance_vars` as the separate `_ignoreOpenCheckCondition` ivar, not `_ignoreCheckCondition`; see finding below |
| 2264 | `-[SCSITape majorDevNum]` | `assembly-matched` | 4-instruction leaf; `lwz r3, 0x110(r3)` |
| 2280 | `-[SCSITape acquireDevice]` | `assembly-matched` (Phase 2) | Finding: calls `[_controller reserveTarget:lun:forOwner:]` and `[self reserveAllLuns]`, neither of which our source calls |
| 2464 | `-[SCSITape releaseDevice]` | `assembly-matched` (Phase 2) | Finding: calls `[self releaseAllLuns]` and `[_controller releaseTarget:lun:forOwner:]`, neither of which our source calls |
...
## Finding: `-[SCSITape setIgnoreCheckCondition:]` writes to `_ignoreOpenCheckCondition`, a distinct ivar 256 bytes past `_ignoreCheckCondition` (repaired)
...
## Finding: `+[SCSITape probe:]` queries the controller for its target count and never reserves a target/lun (repaired)
...
## Finding: `-[SCSITape free]` releases the device through `_devAcquired`, not `_reservedTargetLun` (repaired)
...
## Finding: `-[SCSITape acquireDevice]` reserves the target/lun and all LUNs with the controller; our source only flips a flag (repaired)
...
## Finding: `-[SCSITape releaseDevice]` releases all LUNs and the controller reservation; our source only flips a flag (repaired)
...
## Finding: `cdb_c6s_len_value`'s `__BIG_ENDIAN__` branch returns the array's address instead of reconstructing the 24-bit length (repaired)
...
## Finding: `_st_devsw_init` installs `_enodev`, but our source passes `nodev`, an identifier with no definition anywhere in this tree (repaired)
...
## Finding: `_st_rw`'s read path unconditionally forces `setSuppressIllegalLength:` and `C6OPT_SIL`, with no getter check at all (repaired)
...
## Finding: `_stioctl`'s `MTIOCGET` case writes `MT_ISEXB`, an identifier with no definition anywhere in this tree (repaired)
...
## Finding: `read_er_info_low_24()` is called with no argument in a dead code branch (repaired)
...
## Finding: `_st_doiocsrq` never reads `srp->sr_discon_disable` or `srp->sr_ignore_chkcond`, both of which the reference honours (repaired)
...
## Finding: `_st_doiocsrq` diverges from source for `srp->sr_dma_max == 0`, passing `srp->sr_addr`/`IOVmTaskCurrent()` instead of the declared `NULL` defaults (repaired)
...
| Address | Function | Status | What was compared |
| --- | --- | --- | --- |
| 7424 | `_st_devsw_init` | `assembly-matched` (Phase 2) | Slot-mapping table above (clean); `nodev`/`_enodev` finding above |
...
Five of the nine reached `assembly-matched` in Phase 1; four
(`_st_devsw_init`, `_st_rw`, `_stioctl`, `_st_doiocsrq`) carried findings
above and stayed `unexamined`. Every finding corresponds to exactly one of
those four entries, and every one of those four entries has at least one
finding. Phase 2 repaired all four, so all nine are now `assembly-matched`.
...
## Finding: `_do_ioc` is not absent from source — it matches `stblocksize.c`'s `do_ioc()` exactly, and the "unmapped" appearance in the ledger/source-map is a tooling gap, not a missing function (tooling gap closed)
...
instead of 3 / 17. Task 12 has since regenerated it: `stblocksize/source-map.json`
now reads 4 mapped / 16 unmapped and maps `_do_ioc` to `stblocksize.c:170`, and
the re-seeded ledger takes that `source_path`/`source_line` from the map rather
than from the hand-set values above. Flagging this
...
## Finding: two of the four originally-"absent" methods already exist in source — a `source_map.py` selector-reconstruction bug, not a missing function (tooling gap closed)
...
Task 12 has since regenerated it: `SCSITape/source-map.json` now reads 48
mapped / 2 unmapped, and both selectors resolve to their real names.

Task 12 also found the same comment-stripping defect in `selector_check.py`,
which carried its own private copy of the selector reader. That copy was why
`executeRequest:buffer:/*:senseBuf:` and `initSCSITape:/*:lun:...` appeared in
both the gate's `missing` and `extra` lists. Both readers are now one shared
function in `source_map.py` (`41782e55`), and the gate's `extra` list is empty.

## Finding: `-[SCSITape initSCSITape:target:lun:controller:majorDeviceNumber:]` (address 524, 908 bytes; `SCSITape.m:180-346`) — the transient reserve/release cycle, ivar zeroing, and final `IOLog` all diverge (repaired)
...
## Finding: reconciling `initSCSITape:`'s reservation calls with the existing `probe:`/`acquireDevice` findings — the reference reserves transiently at probe time, not only at open time (analysis reconciliation, no source defect)
...
## Finding: `-[SCSITape executeRequest:buffer:client:senseBuf:]` (address 5836, 832 bytes; `SCSITape.m:963-1105`) — the `rtn == GOOD` if/else sits outside the ignore-check guard, and the reference never returns the controller's own status (repaired)
...
## Finding: correcting the `_didWrite` "unreachable" proof — the WRITE-opcode path is reachable through the ignore-flag exits, not only through `!_isInitialized` (analysis correction, no source defect)
...
## Finding: `-[SCSITape reserveAllLuns]` (address 7012, 236 bytes) — genuinely absent, `void`-returning, declared in a category (written)
...
## Finding: `-[SCSITape releaseAllLuns]` (address 7280, 128 bytes) — genuinely absent, `void`-returning, declared in a category (written)
...
Every one of the twenty findings above was dispositioned. Fourteen were defects
in our source and were repaired; two were absent methods and were written; two
were tooling gaps in `binrecon`; two turned out on re-derivation to be analysis
corrections that required no source change. Nothing here is compile-verified.

**Repaired in source:**

| Finding | Commit |
| --- | --- |
| `+[SCSITape probe:]` queries the controller for its target count and never reserves | `c759a6a8` |
| `-[SCSITape free]` releases through `_devAcquired`, not `_reservedTargetLun` | `92ff0a58` |
| `-[SCSITape setIgnoreCheckCondition:]` writes `_ignoreOpenCheckCondition` | `bacea4dc` (ivar layout), `e91a74f4` (the write) |
| `-[SCSITape acquireDevice]` reserves the target/lun and all LUNs | `92b65f7b` |
| `-[SCSITape releaseDevice]` releases all LUNs and the controller reservation | `70fa3c23` |
| `-[SCSITape initSCSITape:...]`'s reserve/release cycle, ivar zeroing and final `IOLog` | `2dc02771` |
| `-[SCSITape executeRequest:...]`'s status rewriting and ignore-flag guards | `383219cb` |
| `cdb_c6s_len_value` returns the array's address instead of the 24-bit length | `6b7bda1f` |
| `_st_devsw_init` passes `nodev`, an identifier with no definition in this tree | `6ecd7957` |
| `_st_rw` unconditionally forces `setSuppressIllegalLength:` and `C6OPT_SIL` | `693315f5` |
| `_stioctl`'s `MTIOCGET` writes `MT_ISEXB`, undefined in this tree | `6ecd7957` |
| `read_er_info_low_24()` called with no argument | `6ecd7957` |
| `_st_doiocsrq` never reads `sr_discon_disable` or `sr_ignore_chkcond` | `3f724f88` |
| `_st_doiocsrq`'s `sr_dma_max == 0` defaults | `3f724f88` |

**Written (genuinely absent from the tree):**

| Finding | Commit |
| --- | --- |
| `-[SCSITape reserveAllLuns]` (address 7012) | `c44c5c53`, mapped by `5b0c1fe7` |
| `-[SCSITape releaseAllLuns]` (address 7280) | `c44c5c53`, mapped by `5b0c1fe7` |

**Tooling gaps, not source defects:**

| Finding | Commit |
| --- | --- |
| `_do_ioc` appeared `unmapped` because `source_map.py` required K&R parameter lines to be indented | `6e95ae49` (scanner), `9f4a7090` (note) |
| Two "absent" methods appeared absent because `_selector()` read a `/* */` comment as a selector keyword | `a9411a05` (scanner), `9f4a7090` (note), `41782e55` (the same defect in `selector_check.py`'s private copy) |

**Analysis corrections, no source change required:**

| Finding | Why no change |
| --- | --- |
| Reconciling `initSCSITape:`'s reservation calls with the `probe:`/`acquireDevice` findings | Establishes that the reference reserves transiently at probe time; the source change it implies is the one `2dc02771` made |
| Correcting the `_didWrite` "unreachable" proof | Corrects Phase 1's reasoning about why the WRITE-opcode path is reachable; the conclusion, and therefore the source, is unchanged |

**One repair had no Phase 1 finding of its own.** Task 8 also dropped a `[0]`
subscript applied to the scalar `ir_zero3` field in `stInquiry:` (`6ecd7957`).
Phase 1 recorded `stInquiry:` as `assembly-matched` and did not raise this as a
finding; it is a compile defect Task 8 noticed while in the file.

### A note on the intermediate commits

Task 11 committed one function per commit. Commit `bacea4dc` replaces the
scalar `_ignoreCheckCondition` with the reference's check-condition matrix and
drops `_reservedTargetLun`, but the two commits that stop referencing the old
scalar — `e91a74f4` (`setIgnoreCheckCondition:`) and `383219cb`
(`executeRequest:`) — land after it. Commits `bacea4dc`..`e91a74f4` therefore
do not individually compile, even in principle. The branch tip does. This is a
consequence of the one-function-per-commit rule, not a defect in the result,
and it is recorded rather than rebased away: the history is a reviewable record
of how each function was derived, and rewriting it to make each step
independently buildable would cost that without buying anything, since no
PowerPC compiler exists here to build any commit anyway.

## Acceptance

Regenerated from the finished tree (Task 12). **Nothing below is
compile-verified.** There is no PowerPC compiler in this environment, so no
part of `src/drvSCSITape` has been built — including the two bodies Task 9
wrote (`reserveAllLuns`, `releaseAllLuns`) and every repair Task 11 made. Each
gate proves structural correspondence to the reference binary, name agreement,
or artifact well-formedness. None proves the driver compiles, links or runs.

### Regenerated source maps

```
SCSITape     mapped= 48 unmapped=  2 dup=0 bnd=0
PreLoad      mapped=  1 unmapped= 11 dup=0 bnd=0
PostLoad     mapped=  1 unmapped= 15 dup=0 bnd=0
stblocksize  mapped=  4 unmapped= 16 dup=0 bnd=0
```

These match the spec's section 4.2 expectation exactly (`SCSITape` 48/2,
`PreLoad` 1/11, `PostLoad` 1/15, `stblocksize` 4/16, all with 0 duplicates and
0 boundary-disputed). The `unmapped` entries are exactly the out-of-scope
classes and nothing else:

- `SCSITape`: the 2 build-generated classes.
- `PreLoad`: 6 crt/dyld + 5 `__picsymbol_stub`.
- `PostLoad`: 6 crt/dyld + 9 `__picsymbol_stub`.
- `stblocksize`: 6 crt/dyld + 10 `__picsymbol_stub`.

Against the committed pre-regeneration maps, four `SCSITape` entries moved
from `unmapped` to `mapped` — `reserveAllLuns` and `releaseAllLuns` (written
by Task 9) and `initSCSITape:...` and `executeRequest:...` (freed by Task 10's
comment fix) — and `_do_ioc` did the same in `stblocksize` (freed by Task 10's
K&R fix). Thirty-eight `SCSITape` `source_line` values moved to follow the
Phase 2 edits. `PreLoad` and `PostLoad` are unchanged.

### Ledger reconciliation

Each ledger was re-seeded from its regenerated source map with
`seed_ledger.py` (50/12/16/20 entries, all `unexamined`), then every
previously-recorded status was replayed through the `binrecon ledger` CLI one
forward transition at a time — 250 calls, no hand-editing. `source_path` and
`source_line` were deliberately *not* replayed, so each entry takes the anchor
the regenerated map supplies.

**Every entry landed on exactly the status it held before.** No entry's
evidence-supported disposition differs from what the old ledger recorded: the
48 `SCSITape` entries Phase 2 finished are `assembly-matched`, and the 44
out-of-scope entries across all four ledgers keep the `intentional-mismatch`
reasons they were given.

Re-seeding re-anchored **40** `source_line` values that had drifted as Tasks 9
and 11 shifted `SCSITape.m` and `SCSITapeKern.m` — not only the three known
ones (`free` 325 to 349, `acquireDevice` 464 to 472, `executeRequest:` 945 to
963) but every other accessor, command method and C helper below them, plus
two that moved *backwards* in `SCSITapeKern.m` (`_stioctl` 434 to 427,
`_st_doiocsrq` 574 to 567, both pulled up by `693315f5` collapsing eight lines
of `st_rw`'s read path to one) and `initSCSITape:` 184 to 180, whose old value
pointed at a continuation line of the signature rather than its first line. Verified two ways: every ledger
entry's `source_path`/`source_line` now equals its source map's, and reading
each recorded line out of the file on disk shows it contains that entry's own
symbol name, in all 54 mapped cases.

Final status histogram across all four ledgers (98 entries):

| Status | Count |
| --- | --- |
| `assembly-matched` | 54 |
| `intentional-mismatch` | 44 |
| `unexamined` | 0 |

**No entry is `unexamined`,** so the spec's section 4.2 item 4 requirement to
justify each remaining `unexamined` entry is vacuous for this reconstruction.

### Gate results

```
$ PYTHONPATH=tools/binrecon $PY tools/binrecon/selector_check.py "$TAPE/SCSITape_reloc" "$DRV"
reference selectors: 36
our definitions:     34
...
missing (2):
    +[SCSITapeKernelServerInstance kernelServerInstance]
    +[SCSITapeVersion driverKitVersionForSCSITape]

extra (0):
exit=0
```

As expected: `renames (0)`, `duplicates (0)`, exit 0, and `missing` holds only
the two build-generated classes. `extra` is empty, which it was not before
this task: `selector_check.py` carried its own copy of the selector reader
with the same `/* */` defect Task 10 fixed in `source_map.py`, so
`executeRequest:buffer:/*:senseBuf:` and `initSCSITape:/*:lun:...` were
reported as both `missing` and `extra`. Commit `41782e55` deletes that copy
and imports `source_map.py`'s reader, which now also strips nested
parentheses.

```
$ PYTHONPATH=tools/binrecon $PY -c "...load_ledger / load_source_map for all four..."
SCSITape validates
PreLoad validates
PostLoad validates
stblocksize validates
exit=0
```

```
$ PYTHONPATH=tools/binrecon $PY tools/binrecon/ppc_invariant_check.py --binary "$TAPE/SCSITape_reloc"
...
976 fused relocations, 0 violations
exit=0
```

As expected: 976 fused relocations, 0 violations, exit 0.

```
$ PYTHONPATH=tools/binrecon $PY -m pytest tools/binrecon -q
780 passed, 4 skipped in 46.11s
exit=0
```

The suite stood at 773 passed / 4 skipped before this task; the seven new
