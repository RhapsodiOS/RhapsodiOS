...
public final class ExportAnalysis extends GhidraScript {
    ...
    private void prepare(Args args) throws Exception {
        ...
        for (Object item : array(layout.get("symbols"), "symbols")) {
            ...
            String canonicalName=string(symbol.get("name")),internalName=canonicalName;
            if ("external".equals(string(symbol.get("binding"))) && symbol.get("section") == null) {
                currentProgram.getExternalManager().addExtLocation(
                    "UNKNOWN", canonicalName, null, SourceType.IMPORTED);
                internalSymbolNames.put(canonicalName,internalName);
                symbolIndex++;
                continue;
            }
            SourceType source = "local".equals(string(symbol.get("binding")))
                ? SourceType.ANALYSIS : SourceType.IMPORTED;
            Address address = space.getAddress(number(symbol.get("address")));
            try {
                symbols.createLabel(address, internalName, source);
            } catch (InvalidInputException error) {
                internalName="binrecon_symbol_"+symbolIndex;
                symbols.createLabel(address, internalName, source);
            }
            internalSymbolNames.put(canonicalName,internalName);
            symbolIndex++;
    ...
    private void realizeEntryPoints(Map<String,Object> layout,AddressSpace space) throws Exception {
        ...
        for(Object value:array(layout.get("entry_points"),"entry points")){Map<String,Object> entry=object(value,"entry point");
            ...
            String entryName=string(entry.get("name"));String internalName=internalSymbolNames.getOrDefault(entryName,entryName);
            Function function=functions.getFunctionAt(address);if(function==null)function=createFunction(address,internalName);
    ...
    private List<Object> exportFunctions(List<Object> summaries) throws Exception {
        ...
        try {
            ...
            for(Function function:functions){
                monitor.checkCancelled(); Map<String,Object> item=map();
                item.put("address",function.getEntryPoint().getOffset());
                item.put("size",function.getBody().getMaxAddress().subtract(function.getEntryPoint())+1);
