...
_NATIVE_SECTION_OVERLAP = re.compile(
    r"^section .+ at address \d+ overlaps section .+ at address \d+$"
)
...
def _layout(profile, identity: InputIdentity) -> dict:
    ...
    segment_metadata = list(extension.get("segments", ()))
    ...
    if configured_regions:
        ...
        for source in configured_regions:
            ...
            if matches:
                ...
                segments = [item for item in segment_metadata
                            if tuple(item[field] for field in identity_fields) == identity_key]
                if len(segments) != 1:
                    raise GhidraAdapterError(
                        f"fallback section metadata is missing or ambiguous: {source['name']!r}"
                ...
                matches = [item for item in section_metadata
                           if item.get("segment_ordinal") == segments[0].get("ordinal")]
                if not matches:
                    raise GhidraAdapterError(
                        f"fallback segment has no canonical sections: {source['name']!r}"
                    )
            for metadata in matches:
                if metadata["ordinal"] in selected_ordinals:
                    raise GhidraAdapterError(
                        f"fallback section metadata is ambiguous: {source['name']!r}"
                    )
                selected_ordinals.add(metadata["ordinal"])
                core_key = tuple(metadata[field] for field in identity_fields)
                core_matches = core_by_identity.get(core_key, [])
                if not core_matches:
                    raise GhidraAdapterError(
                        f"fallback section lacks canonical Mach-O identity: {metadata['name']!r}"
                    )
                effective = dict(core_matches[0])
                effective["permissions"] = source["permissions"]
                selected.append((effective, metadata))
    else:
    ...
    try:
        ...
        if completed.returncode == 0 and output.is_file() and not native_needs_fallback:
            ...
            try:
                ...
                native_needs_fallback = _NATIVE_SECTION_OVERLAP.fullmatch(str(error)) is not None
            except Exception:
                # Final validation below reports every non-overlap failure without retrying.
                pass
