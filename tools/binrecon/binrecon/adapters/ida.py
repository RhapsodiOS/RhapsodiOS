...
def _script_command(script: Path, output: Path, identity: InputIdentity,
                    mapping: Path, mapping_sha256: str) -> str:
...
def _mapping_manifest(profile, identity: InputIdentity) -> dict:
    ...
    except (OSError, MachOFormatError, KeyError, TypeError, ValueError):
        runs = [{key: item[key] for key in ("address", "offset", "size")}
                for item in profile.document.get("regions", ())]
    runs = sorted((item for item in runs if item["size"]),
