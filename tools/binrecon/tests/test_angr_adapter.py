...
def test_host_layout_preserves_canonical_relocation_metadata(tmp_path):
    ...
    layout = angr_host._layout(profile, identity)

    assert layout["relocation_metadata"][0]["original_bytes"] == "00000000"
...
@pytest.mark.skipif(importlib.util.find_spec("angr") is None, reason="angr not installed")
def test_capstone_exports_structured_absolute_memory_and_immediate_operands(tmp_path):
    module = _load_script(); import angr
    binary = tmp_path / "operands.bin"
    binary.write_bytes(bytes.fromhex("8B1508A10000B8D0A30000C3"))
    project = angr.Project(str(binary), main_opts={"backend": "blob", "arch": "x86",
        "base_addr": 0, "entry_point": 0}, auto_load_libs=False)

    _, cfg, _ = module.export_cfg(project,
        {"entry_points": [0], "sections": [{"address": 0, "size": 12,
            "permissions": "rx"}]}, {"symbols": [], "relocations": []})

    assert cfg["instruction_operand_metadata"][:2] == [
        {"address": 0, "operands": [
            {"index": 0, "kind": "register"},
            {"index": 1, "kind": "memory", "encoded_address": 0xA108,
             "field_offset": 2, "field_width": 4}]},
        {"address": 6, "operands": [
            {"index": 0, "kind": "register"},
            {"index": 1, "kind": "immediate", "resolved_address": 0xA3D0,
             "field_offset": 1, "field_width": 4}]}]


@pytest.mark.skipif(importlib.util.find_spec("angr") is None, reason="angr not installed")
@pytest.mark.parametrize("code,expected", [
    ("8B0C9D18800000C3", {"index": 1, "kind": "memory", "encoded_address": 0x8018,
                         "field_offset": 3, "field_width": 4}),
    ("8B4020C3", {"index": 1, "kind": "memory", "encoded_address": 0x20,
                   "field_offset": 2, "field_width": 1}),
    ("8B40F8C3", {"index": 1, "kind": "memory", "encoded_address": 0xF8,
                   "field_offset": 2, "field_width": 1}),
    ("678B86F8FFC3", {"index": 1, "kind": "memory", "encoded_address": 0xFFF8,
                       "field_offset": 3, "field_width": 2}),
    ("A100000000C3", {"index": 1, "kind": "memory", "encoded_address": 0,
                       "field_offset": 1, "field_width": 4}),
    ("8B00C3", {"index": 1, "kind": "memory"}),
])
def test_capstone_exports_only_encoded_memory_displacement_fields(
        tmp_path, code, expected):
    module = _load_script(); import angr
    binary = tmp_path / "indexed.bin"
    binary.write_bytes(bytes.fromhex(code))
    project = angr.Project(str(binary), main_opts={"backend": "blob", "arch": "x86",
        "base_addr": 0, "entry_point": 0}, auto_load_libs=False)

    _, cfg, _ = module.export_cfg(project,
        {"entry_points": [0], "sections": [{"address": 0, "size": len(bytes.fromhex(code)),
            "permissions": "rx"}]}, {"symbols": [], "relocations": []})

    assert cfg["instruction_operand_metadata"][0]["operands"][1] == expected


@pytest.mark.skipif(importlib.util.find_spec("angr") is None, reason="angr not installed")
def test_capstone_omits_encoded_field_metadata_for_implicit_immediate(tmp_path):
    module = _load_script(); import angr
    binary = tmp_path / "implicit.bin"
    binary.write_bytes(bytes.fromhex("D0E8C3"))  # SHR AL, 1
    project = angr.Project(str(binary), main_opts={"backend": "blob", "arch": "x86",
        "base_addr": 0, "entry_point": 0}, auto_load_libs=False)

    _, cfg, _ = module.export_cfg(project,
        {"entry_points": [0], "sections": [{"address": 0, "size": 3,
            "permissions": "rx"}]}, {"symbols": [], "relocations": []})

    assert cfg["instruction_operand_metadata"][0]["operands"][1] == {
        "index": 1, "kind": "immediate"}
