...
Assert-NotMatch $rbuildBlacklistText 'drvEIDE' 'working i386 EIDE is not blacklisted'
Assert-Match $rbuildRunnerText '(?s)int runner_buildpackage.*?buildpackage_for_arch' 'standalone buildpackage delegates to the per-architecture builder'
Assert-Match $rbuildRunnerText '(?s)static int buildpackage_for_arch.*?opt\.clean = 1' 'standalone buildpackage removes package chroots'
